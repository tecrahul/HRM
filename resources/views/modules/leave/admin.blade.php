...
@section('content')
    ...
    <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <article class="ui-section xl:col-span-2">
            ...
            <form method="GET" action="{{ route('modules.leave.index') }}" class="mt-4 grid grid-cols-1 md:grid-cols-6 gap-3">
                <input
                    type="text"
                    name="q"
                    value="{{ $filters['q'] }}"
                ...
                <select name="leave_type" class="ui-select">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($leaveRequest->status === 'pending')
                                    <form method="POST" action="{{ route('modules.leave.review', $leaveRequest) }}" class="flex items-center justify-end gap-2">
                                        @csrf
                                        @method('PUT')
                                        <input type="text" name="review_note" placeholder="Review note (required for reject)" class="ui-input w-52 !py-1.5 !text-xs">
                                        <button type="submit" name="status" value="approved" class="ui-btn !py-1.5 !text-xs" style="background: #16a34a; color: #fff; border-color: transparent;">Approve</button>
                                        <button type="submit" name="status" value="rejected" class="ui-btn !py-1.5 !text-xs" style="background: #dc2626; color: #fff; border-color: transparent;">Reject</button>
                                    </form>
                                @else
        ...
        <div class="space-y-5">
            <article class="ui-section">
                ...
                <form method="POST" action="{{ route('modules.leave.store') }}" class="mt-4 grid grid-cols-1 gap-3">
                    @csrf

                    <div>
                        <label for="assign_user_id" class="ui-kpi-label block mb-2">Employee</label>
                        <select id="assign_user_id" name="user_id" class="ui-select">
