...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <form action="{{ route('settings.roles-permissions.sync-users') }}" method="POST" class="inline">
                    @csrf
                ...
                <form action="{{ route('settings.roles-permissions.clear-cache') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('settings.roles-permissions.health') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-3xl font-semibold text-gray-900">{{ $totalPermissions }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-3xl font-semibold text-gray-900">{{ $roles->sum('users_count') }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white shadow overflow-hidden rounded-lg mb-8">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p class="mt-1 text-sm text-gray-500">Manage user roles and their associated permissions</p>
        </div>
                    ...
                    @forelse($roles as $role)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center">
                                        <span class="text-indigo-600 font-semibold text-sm">{{ strtoupper(substr($role->name, 0, 2)) }}</span>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ ucwords(str_replace('_', ' ', $role->name)) }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('settings.roles-permissions.show-role', $role) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                <a href="{{ route('settings.roles-permissions.edit-role', $role) }}" class="text-indigo-600 hover:text-indigo-900">Edit Permissions</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="bg-white shadow overflow-hidden rounded-lg">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p class="mt-1 text-sm text-gray-500">View all permissions organized by module</p>
        </div>
                ...
                @foreach($permissionsByModule as $module => $modulePermissions)
                    <div class="border border-gray-200 rounded-lg p-4">
                        <h4 class="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                            ...
                            {{ ucfirst($module) }}
                            <span class="ml-auto text-xs text-gray-500">({{ $modulePermissions->count() }})</span>
                        </h4>
                        <div class="space-y-1">
